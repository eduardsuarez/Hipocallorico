 ...
 */
function listarxEstado() {
  let estadoOrden = $("#estadoOrden").val();

  $.ajax({
    // la URL para la petición (url: "url al recurso o endpoint")
    url: `http://localhost:8090/api/order/state/${estadoOrden}/${userId}`,
    ...
    success: function (respuesta) {
      ...
      console.log(userId);
      listarxEstadoProductos(respuesta);
    },
...
function listarxEstadoProductos(items) {
...
//cuando carga la página html se ejecuta la función: listarxEstado()
$(document).ready(function () {
  //ejecuta función para enviar petición al ws
  estadoInicialxEstado();
